...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    // Vous pouvez ajouter optionalement availableFrom et/ou availableUntil (formats variés acceptés)
    const products = [
      {
        id: 'metamorphosis',
        name: 'Metamorphosis',
    ...
    function parseDateOrNull(dateInput) {
      ...
      // 1) Try native parse (ISO)
      let t = Date.parse(s);
      if (Number.isFinite(t)) return t;
      // 2) Try replace space with T (YYYY-MM-DD HH:MM)
      t = Date.parse(s.replace(' ', 'T'));
      if (Number.isFinite(t)) return t;
      // 3) French format DD/MM/YYYY [HH:MM(:SS)]
    ...
    function getVisibleProducts(nowMs = Date.now()) {
      // On applique d'abord les filtres (filtered), puis la contrainte date
    ...
    function renderProducts() {
      ...
      visible.forEach(p => {
        ...
        // lien par défaut sur la première couleur (sécurisé)
