...
// --- Ajout de l'endpoint pour Stripe Elements ---
app.post('/create-payment-intent', async (req, res) => {
  try {
    ...
    // Calcul du montant en centimes (exemple simple, adapter selon votre logique)
    const amount = items.reduce((sum, item) => sum + Math.round(item.price * 100) * item.quantity, 0);

    // Création du PaymentIntent
    const paymentIntent = await stripe.paymentIntents.create({
      amount,
      currency: 'eur',
      // Pour pouvoir récupérer le client_secret dans le front
